<template>
  <div class="w-full">
    <h1 class="mb-10 text-4xl">Información de su cuenta</h1>
    <input
      class="w-1/2 h-12 mb-2 text-lg text-gray-700 placeholder-gray-600 border rounded-lg  focus:shadow-outline"
      type="number"
      v-model="addedMoney"
      name="moneyAccount"
      id=""
    ...
    <p>
      Cuenta corriente: <span>{{ moneyAccountValue }}</span>
    </p>

    <div v-show="account.length > 0" class="w-full py-6">
      <input
        class="w-1/2 h-12 mb-2 text-lg text-gray-700 placeholder-gray-600 border rounded-lg  focus:shadow-outline"
        type="text"
        v-model="filters.assetName.value"
      ...
      <div class="overflow-hidden border-b border-gray-200 rounded shadow">
        ...
        >
          <thead slot="head" class="text-white bg-gray-700">
            <v-th
              class="px-2 py-2 text-sm font-semibold text-left uppercase"
              sortKey="assetName"
              defaultSort="desc"
            >
              Nombre producto
          ...
          <tbody slot="body" slot-scope="{ displayData }" class="text-gray-700">
            <tr
...
export default {
  ...
  methods: {
    ...
    getProfit() {
      axios.get("/getProfit").then((response) => (this.profit = response.data));
    },
